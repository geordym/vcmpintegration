...
public class VehicleImpl extends EntityImpl implements Vehicle {
    ...
    public float getBasicAngle() {
        float angle = (float) (Math.asin(this.getRotation().z) * -2);
        angle = (float) (this.getRotation().w < 0 ? Math.PI - angle : 6.28319f - angle);
        return angle;
    }

    @Override
    public Vector getPositionInfront(float distance) {
        float newX = (float) (this.getPosition().x - distance * Math.sin(this.getBasicAngle()));
        float newY = (float) (this.getPosition().y + distance * Math.cos(this.getBasicAngle()));
